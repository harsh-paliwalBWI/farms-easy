...
const FromFarmacy = () => {
  ...
  return (
    <div className='sm:px-[3.5%] px-[7%]  py-[20px] relative'>
        <div className='text-center font-bold sm:text-4xl text-2xl'>Form Farmacy Farm Fresh</div>
            ...
            <div className='  flex flex-col sm:gap-[60px] gap-[30px]'>
            {
                DUMMY_DATA.map((item:any,idx:number)=>{
                    return <div className='flex items-center gap-5' key={idx}>
                        <div className=''>
                        <div className='font-bold sm:text-lg text-base text-right mb-[5px]'>{item.heading}</div>
                        ...
                        {/* <div className=''>
                            <Image src={item.image} alt='' height={50} width={50}/>
                             ...
                             <div className='relative  z-30'><Image src={item.image} alt='' height={50} width={50}/>
                        <div className='h-[40px] w-[40px] rounded-full bg-[#EFF4DC] absolute top-[50%] left-[50%] z-10 opacity-50 '></div>
                        </div>
                    </div>
                })
            }
            </div>
            <div><Image src={HeartVegetableImg} alt='' height={550} width={550}/></div>
            <div className='  flex flex-col sm:gap-[60px] gap-[30px]'>
            {
                DUMMY_DATA1.map((item:any,idx:number)=>{
                    return <div className='flex items-center gap-5' key={idx}>
                        <div className='relative  z-30'><Image src={item.image} alt='' height={50} width={50}/>
                        <div className='h-[40px] w-[40px] rounded-full bg-[#EFF4DC] absolute top-[50%] right-[50%] z-10 opacity-50 '></div>
                        </div>
                        <div className=''>
                        <div className='font-bold sm:text-lg text-base mb-[5px]'>{item.heading}</div>
        ...
        {!matches&&
        <div className='absolute right-0 top-0'><Image src={blinkImg} alt=''/></div>}
    </div>
  )
}
