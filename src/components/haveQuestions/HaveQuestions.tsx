...
const DUMMY_DATA = [
  {
    ...
    answer: "Lorem ipsum dolor sit amet, consectetur adipiscing elit eiusmod.",
  },
  {
    question:
      "Lorem ipsum dolor sit amet, consectetur adipiscing elit eiusmod.",
...
const HaveQuestions = () => {
  ...
  return (
    <div
      className={`bg-have-questions-bg bg-cover bg-no-repeat flex md:flex-row flex-col gap-[60px]   items-center ${
        matches ? "py-[30px]" : "py-0"
      } ${matches2 ? "px-[7%]" : matches ? "px-[3.5%]" : "pr-[3.5%]"}`}
    >
      {!matches && (
        <div className="w-1/2">
      ...
      <div className={`${matches ? "w-[100%]" : "w-1/2"}`}>
        <div className="xl:text-4xl text-2xl font-bold mb-[10px] sm:text-start text-center ">
          Have Questions?
        </div>
        {DUMMY_DATA.map((item: any, idx: number) => (
          <Disclosure key={idx}>
            ...
            <Disclosure.Panel className="text-gray-500 w-full text-xs bg-gray-50 py-2 pl-4">
              {item.answer}
            </Disclosure.Panel>
          </Disclosure>
        ))}
